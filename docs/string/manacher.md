...
* $s_i = \frac{t_i}{2} - 1$
* $[L_{t_i}, R_{t_i}] = [t_i - hl + 2, t_i + hl - 2] \rightarrow [L_{s_i}, R_{s_i}] = [\frac{t_i - hl}{2}, \frac{t_i + hl}{2} - 2]$
    由於在間隔中插入 "#"，所以若是已經是回文子串，會因為插入的 "#" 往外再擴張，所以得到的 $[L, R]$ 要減去最外面的 "#"，才是對應到原本字串字符的位置。

* 一般用來計算 $t_i$ 的範圍 $[2, t.size() - 2 - 1]$，扣掉額外加入的部分。
...
//改成不會出現的元素
const int padding = -1;
const int dummy_start = -2;
const int dummy_end = -3;
long long getSum(vector<int>& s) {
    ...
    t.push_back(dummy_start);
    for (int x : s) {
        t.push_back(padding);
        t.push_back(x);
    }
    t.push_back(-1);
    t.push_back(dummy_end);
