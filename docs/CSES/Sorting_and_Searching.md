...
一開始聯想到了配對貪心，但是又有點不一樣，因為最後都需要讀完全部的書，所以答案至少會是 $\sum{b_i}$，所以關鍵是在什麼情況下所需要的時間會增加。

wa 了一次，使用的想法是將陣列由大至小排序，然後依次分給 A, B 兩個子集，用最後一個元素判斷是否要用 $\sum{b_i}$ 或是 子集和 * 2，這個方法是錯的。

觀察範例，時間會增加的原因是其中一人出現了不能讀書的空檔，這樣的情況會發生在只有一個元素特別大的情況，其他情況都可以透過調整順序讀其他書，當元素大到比其他元素總和還大時，此時我們只能等最大的書讀完因此所需時間為最大元素乘 2。
...
一開始會將 [3, 4], [2, 5] 加入佇列，遇到 [6, 7] 因為佇列中的電影都結束了所以彈出[3, 4], [2, 5]，並接著加入 [6, 7], [1, 8], [9, 10]。問題出在加入 [1, 8]，此時我們的開始時間是 t = 6 不應該允許加入 [1, 8]，因為前面 [3, 4], [2, 5]，已經看過表示已經錯過開始時間了。

由此可以感覺得到，似乎要透過開始順序的方式來加入答案，但是這樣會導致先看了比較長的電影無法選擇更多比較短的電影，但若我們可以已經選擇的電影換成比較短的電影，在相同貢獻下將結束時間儘可能地縮短，以容納下部電影的加入。
