...
想到這邊 -> 又回到上面的問題核心 -> "能否兩兩配對?" -> 判斷最多頻率的元素是否超過整體長度的 $\frac{1}{2}$  -> 判斷"剩餘"最多頻率的元素是否超過"剩餘"長度的 $\frac{1}{2}$

若是在我們填字母的過程中保證 "剩餘"最多頻率的元素不超過"剩餘"長度的 $\frac{1}{2}$，則我們便可以按照字母順序構造答案。

此外因為我們是一個一個填字母，最大值會的變化會是連續整數，所以可以只為維護最大值和次大值，進行判斷，不需要搜索26個字母。
