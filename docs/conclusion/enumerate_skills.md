...
\[
\begin{array}{ll}
    \sum_{i = m}^{n}a_ib_i &=a_nS_n - \sum_{i = m}^{n-1}(a_{i + 1} - a_i)S_i\\
    S_i &= \sum_{k = m}^{i} b_k
\end{array}
\]
...
不要一根一根 $b_i$ 看 => 改成看**前綴 ($S_i = \sum{b_i}$)** => 每次**權重變化**多少，就把**整塊前綴和乘積變化量**一起扣掉:
...
* [3500. 将数组分割为子数组的最小代价](https://leetcode.cn/problems/minimum-cost-to-divide-array-into-subarrays/description/?envType=problem-list-v2&envId=n91HJ7EL)

    改寫式子:
    1. $(nums[1] + ... + nums[r]) * (cost[l] + ... + cost[r])$
    2. $k * i * (cost[l] + ... + cost[r])$

    第一個式子是劃分型 DP ，第二個式子因為有 i 導致不好計算。
    ...
    由於每次 i 的變化量只有 1 也就是 $a_{i + 1} - a_i = 1$，因此上面的求和公式可以寫成
