...
* [link](https://leetcode.cn/problems/earliest-second-to-mark-indices-ii/description/)
...
1. 對於 nums[i] > 1，盡可能使用 op2 ，對同一個 i 使用 op1 後再使用 op2 會浪費。

2. 對於 nums[i] <= 1，使用 op2 和 op1 的花費是一樣的，因此盡量使用 op1。

3. op3 要盡量靠後

4. 要先將 nums[i] 歸 0 不然無法操作 op3
...
* 單調性:
合法答案的值域在 $[1, m]$ 之間，在值域範圍內若是答案秒數越多越有機會符合題目要求。
...
* 不能做 op2:
    1. 那就將 free += 1 表示可以用來做 op1 or op3
...
* 可以做 op2:
    ...
    * 不做 op2 的情況:
        ...
        2. nums[i] = 1
        3. free = 0 無法做 op3
         $\rightarrow$ 思考是否這個 i 只能使用 op1 完成?
         $\rightarrow$ 將之前使用過 free 的人替換改成自己，被替換的人使用 op1 
         $\rightarrow$ 反悔貪心

* 結論:
這題有三個重要的性質要觀察到才能順利解決:
    1. <span style="color:red">單調性</span>
    2. 必須使用<span style="color:red">倒序遍歷</span>，使用二分答案的 check 函數是判定問題，對於順序與逆序不影響判定。
    3. <span style="color:red">反悔貪心</span>的思考過程

* $O(m\log{m})$ 的 [方法](https://leetcode.cn/problems/earliest-second-to-mark-indices-ii/solutions/2866693/wu-xu-er-fen-da-an-de-tan-xin-fa-fu-za-d-1y5u/) ，可以看懂方法，但是有點像在湊答案沒有辦法理解透徹。
...
* [link](https://leetcode.cn/problems/maximum-number-of-non-overlapping-substrings/description/)
...
如果區間內有其他字母就建一條邊，因此若是有這種 abcba 這種例子圖會長這樣 $a \rightarrow b \rightarrow c$，會有三個區間 $[0, 4], [1, 3], [2, 2]$
