...
??? note "Details"
    ...
    這題是上一題的困難版，在寫簡單版本的時候有想到正確的想法但是能力不足沒有實做出來所以選了另一個方式。
    
    簡單思路就是對於最高位的部分可以找到其 hi bit，而我們可以將 $\ge 2^{hi}$ 的數分配給 $2^{hi} - 1$ 往下數的部分，可以發現除了 hi bit 之外剛好互補。之後剩下來的部分也按照同樣方式分配，但是如果只剩下相同 hi bit 的數可以將 hi bit 去除後做同樣的操作，最後再加回來，這邊是我卡住的地方，不知道如何去除後又加回來，實作能力不足。

    * 可以將 $\ge 2^{hi}$ 的數分配給 $2^{hi} - 1$ 往下數的部分
    ...
    * 上面這個方法有一個問題是若是 $len(r - 2^{hi} - 1) > len(2^{hi} - 1 - l + 1)$ 時， $\le 2^{hi} - 1$ 的數會不夠分配給 $\ge 2^{hi}$ 的數，所以這時要用 $\le 2^{hi} - 1$ 分配給 $\ge 2^{hi}$。
    ...
    * 如果只剩下相同 hi bit 的數可以將 hi bit 去除後做同樣的操作，最後再加回來
    ...
    */
    ...
    [code](https://codeforces.com/contest/2146/submission/345388224)
...
??? note "Details"
    ...
    很快發現到最多只需要三種顏色可以把環形陣列區隔開來，如果長度為偶數可以用 1,2,1,2 這種排列，如果是奇數則可以將最後一個元素替換成 3。
    但是因為是環形陣列，所以想的過程中有思考到能否將陣列攤平，其中就有想到若是連續的元素可以分別將兩個元素指派為開頭和結尾，但是這個想法最後沒有走到答案!

    * <span style="color:red">分類討論的能力不足</span> 

    * 首先是不同元素數量 <= 2 的情況
    * 接著是當種類 >= 3 時，陣列長度為奇數時，有沒有連續元素的情況

### 1396B. Stoned Game

??? note "Details"

    一開始直接兩兩配對的想法有出現在腦中，但是題目有一個沒辦法重複選相同元素的限制，所以若是有一個元素比其他元素還要多很多，可以一開始就佔據這個元素直到最後。接著我的思路就開始歪掉，為了找一個相對合理的策略，我想到將整個陣列排序，抽象的看成一層一層的金字塔每次都將每層的元素兩兩配對，剩下的分配給下一層，直到最後在特判最後剩下的元素。

    Solution:

    一開始的"若是有一個元素比其他元素還要多很多，可以一開始就佔據這個元素直到最後" 這個想法是沒問題的，但是寫題目的時候沒有準確的記錄下來是要多少才能保證獲勝 -> 假設元素和為 S ，如果有一個元素 >= S / 2 那 T 必贏。

    接著要思考每個元素皆不超過 S / 2 的情況。

    題解有兩種[證明](https://chatgpt.com/c/6914268a-005c-8323-8bd8-8354a29b1b32)方式

    我比較喜歡歸納法的方式，也是應該要練習的思路:
