...
??? note "Details"
    ...
    這提示上一提的困難版，在寫簡單版本的時候有想到正確的想法但是能力不足沒有實做出來所以選了另一個方式，簡單思路就是對於最高位的部分可以找到其 hi bit，而我們可以將 $\ge 2^{hi}$ 的數分配給 $2^{hi} - 1$ 往下數的部分，可以發現除了 hi bit 之外剛好互補。之後剩下來的部分也按照同樣方式分配，但是如果只剩下相同 hi bit 的數可以將 hi bit 去除後做同樣的操作，最後再加回來，這邊是我卡住的地方，不知道如何去除後又加回來，實作能力不足。
