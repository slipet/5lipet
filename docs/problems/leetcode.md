...
* Comment:
    ...
    而這題則是要把不同電池的時間分配給 n 台電腦，可以把每個電池的時間一個一個時間單位的做切分，用配對貪心的思路去思考，跟配對貪心的問題的差別在每個配對要 n 個不同元素，因此可以知道配對數量的上限為 $\lfloor \frac{sum}{n} \rfloor$，由此可以知道若有一個元素超過 $\lfloor \frac{sum}{n} \rfloor$ ，多出來的部分無論怎麼分配都無法加到最終的分配裡(因為最終分配只有 $\lfloor \frac{sum}{n} \rfloor$)。
