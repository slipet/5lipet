...
* Comment:
    ...
    計算貢獻的方式為，假設目前為 i-th，那左邊符合題目要求且為 1 的貢獻為 $pre_1 \times 2^i$。而一旦 i-th 設為 1 ，那 i-th 為 0 的部分則要計算 $[0, 2^i - 1]$ 與前綴產生的貢獻。這部分是計算右邊 $[0, 2^i - 1]$ 的貢獻，此時貢獻為 $\lflorr \frac{i}{x} \rfloor \times 2^{i - 1}$。可以看成每 x 位可以產生的貢獻為 $2^{i - 1}$ ，因為對於 x 位之外的位置只有 0, 1 兩種可能，又因總共只有 $\lflorr \frac{i}{x} \rfloor$ 個位置。最後得到 $cnt = pre_1 \times 2^i + \lflorr \frac{i}{x} \rfloor \times 2^{i - 1}$。
