...
1. 注意 `dis` 初始化的 inf 大小
2. 網格圖的話注意網格奇偶性
3. 抵達終點步數是否固定 -> BFS
4. 邊權是否皆是 0/1
5. 透過加邊的方式建圖，以及透過限制邊的方式剪枝

##### 最短路上做操作

如果要在最短路上做操作，透過從 **終點** 開始做 BFS/DFS 往回搜索可以成為最短路上的邊，用邊來更新點上的 DP 值。

* **不能從起點 0 出發去尋找在最短路上的邊**
    從起點 0 出發，當發現 dis[x]+w=dis[y] 時，這僅僅意味著 x-y 這條邊在從 0 出發的最短路上，但這條最短路不一定通向終點

##### dijkstra 跟 BFS 的關係

* 無論你走的是哪一條路，如果抵達終點的 **步數** 都一樣，那可以使用 BFS 取代 dijkstra
  如下圖，如果 $A \rightarrow D$ 的距離不管走哪條都是 2 的話，使用 BFS 即可。
...
* 邊權只有 0/1 時
    如果求最短路時，路徑上的邊權為 0 or 1 可以將求最短路改為 0-1 BFS。