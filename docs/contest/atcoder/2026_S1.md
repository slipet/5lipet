...
這題就是樹的直徑。唯一問題就是是計算兩端連結的時候，應該是 $cnt(child) \ge 4$ 就可以連接，因為這種情況不需要傳遞給上面。
...
void solve() {
    ...
    auto dfs = [&](this auto &&dfs, int x) -> void {
        ...
        for(auto &y: g[x]) {
            ...
            dfs(y);
            if(sz >= 4) {
                ans = max(ans, mxchild + f[y] + 1);
            }
            mxchild = max(mxchild, f[y]);
        }
        if(sz >= 3) {
            f[x] = max(f[x], 1);
            ans = max(ans, mxchild + 1);
        }
        if(sz >= 4) f[x] = max(f[x], mxchild + 1);
        return;
    };
    dfs(1);
    cout<<ans<<endl;
...
這題的關鍵點是在題目的特殊限制 $X_i < Y_i$，由題意可以把範例 1 作圖:
...
設第 $i$ 個節點為 $x_i = i$，當 $x_i > x_j$ 的時候 $[x_i, x_j]$ 一定連通，因為依照題意 $i + 1$ 到 $i$ 一定有連邊，所以可以 $x_i \rightarrow x_{i+1} \rightarrow ... \rightarrow x_j$。

可以發現若是 $x_1, x_n$ 連通，則確認此圖一定是強連通，對於 $x_i < x_j$ 的情況，可以 $x_i \rightarrow x_1 \rightarrow x_n \rightarrow x_j$。

至此可以發現問題轉化成 $1 \rightarrow n$ 有多少種方案數，定義 $f(i, r)$ 表示在前 $i$ 條時可以到達 $r$ 的方案數。可以看成區間覆蓋的方案數。

對於第 $i$ 條邊，設 $edge_i = (X_i, Y_i)$，對於 $edge_i$ 有選或不選兩種可能，我們可以依照不同情況有不同的轉移方程:

```
當 f(i, r) 時，對於 edge[i] = (X, Y)
1. X > r 時 => |1------r| |X-----Y|
    選: f(i, r) = f(i - 1, r)
  不選: f(i, r) = f(i - 1, r)
  => f(i, r) = 2f(i - 1, r)
* 選或不選，都只能由 1 至 r ，不影響連通。
...
由上面的情況可以知道只有在 $X \le r \le Y$ 的時候要轉換一下，轉換成 $f(i, r)$:

\[
f(i, r) = \left\{ 
\begin{array}{ll}
    2f(i - 1, r), &\text{ where } r < X_i\\
    2f(i - 1, r), &\text{ where } r > Y_i\\
    f(i - 1, r), &\text{ where } X_i \le r < Y_i \\
    f(i - 1, r) + \sum_{r'=X_i}^{Y_i} f(i - 1, r'), &\text{ where } r = Y_i
\end{array}
\right.
\]

由上面的轉移方程式可以知道需要的操作為區間乘法，單點更新，使用線段樹 + lazy tag 可以達成。
```cpp
void solve() {
    int n, m;
    cin >> n >> m;
    LazySegmentTree<ll> t(n + 1, 0);
    vector<pii> a(m);
    for(int i = 0; i < m; ++i) {
        auto &[x, y] = a[i];
        cin >> x >> y;
...
令 $X_k$ 為所有恰好有 $k$ 位數的好整數集合，並令 $P_k$ 為所有恰好有 $k$ 位數的 2 的冪次集合。為了方便起見，令 $X_0 = \{0\}$。那麼對於所有 $k \ge 1$：
...
\begin{array}{ll}
    S_k - S_{k-1} &= |X_k| \\
    &= |\bigcup_{i=1}^{k} \{x \cdot 10^i + p \mid x \in X_{k-i}, p \in P_i \}| \\
    &\le \sum_{i=1}^{k} |\{x \cdot 10^i + p \mid x \in X_{k-i}, p \in P_i \}| \\
    &= \sum_{i=1}^{k} |X_{k - i}| \cdot |P_i| \\
    &\le 4(|X_{k - 1}| + .. + |X_1| + |X_0|) = 4(S_{k - 1} + 1)
\end{array}
\]
...
\[
\begin{array}{ll}
    S_k - S_{k-1} &\le 4(S_{k - 1} + 1) \\
    S_k &\le 5S_{k - 1} + 4 \\
    S_k + 1 &\le 5(S_{k - 1} + 1) \le 5^2(S_{k-2} + 1) \le ..\le 5^k(S_0 + 1) = 5^k
\end{array}
\]
...
這題我原本的想法是在反斜線上找一個最短的邊當作樹邊，然後判斷構造出來的樹合不合法。判斷的方式為使用 lca 倍增判斷。這個方法有一個問題是假設 $1, 2$ 是一定連在一起的，一旦構造出一顆 $1, 2$ 不連在一起的樹就會出錯。 EX:
...
真正的作法是觀察到對於相鄰節點，他們的距離一定會是最短的，因此把這些邊連起來就會構成一棵樹，也就是 MST 。用這個 MST 對每個點做 BFS 判斷是否合法。

雖然題目給的是兩點之間的距離，但是可以把它看成完全圖來構造 MST。
