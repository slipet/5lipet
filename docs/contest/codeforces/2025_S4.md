...
這題要用貪心的方式由左往右確認每個點的半徑範圍，一但有非法的範圍就斷開重新計算。

每個點的最大範圍會是其左右兩點距離的最小值，而其圓範圍會是 $(d_i - r, d_i - l)$ 。

要注意的是左端點的最小值為 0, 右端點最大值為 $d_i$

---

## [Good Bye 2025](https://codeforces.com/blog/entry/149548)

### C. First or Second

先定義題目的兩種選擇:

* type1: 選第一個元素得到 $x$ 的貢獻

* type2: 選第二個元素得到 $-x$ 的貢獻

一開始發現最後會剩下一個元素，因此一直往剩下這一個元素的方向思考，接著想到有可能是前後綴分解，但是 wa  了一次，中間有想過是不是 DP 的方式，但是沒有很好的轉移方程。突破點在發現對於剩下的那一個元素 y 如果要選其後面的元素，必須使用 type2 操作，因為前面卡了一個 y ，所以 y 後面的貢獻為 $sum - pre - y$。

解決了後綴部分，接著思考 y 前面的前綴該如何求得貢獻，自己模擬了幾次操作後發現，前綴的元素是必須全部選擇的，因此第一個元素只能使用 type 1 操作，對於 $i \in [1, y)$ 的元素可以用 type1 或是 type2 的方式貪心的得到 $> 0$ 的貢獻，所以可以用前後綴分解的方式枚舉 y 找最大的答案。

### D. Xmas or Hysteria

這題就是爐石牧師的群體混亂，整體的主要核心為相鄰不同的配對貪心，比賽的時候沒有想清楚不合法的部分，想成 m > n - 2，但是因為每個元素都互不相同，所以每當兩兩進行攻擊最後會剩下的只有生命值比較高的那一個，因此最多只會剩下 $\lfloor \frac{n}{2} \rfloor$ 個人。

對於 $1 \le m \le \lfloor \frac{n}{2} \rfloor$ 的情況:

思考當 m = 1 的特殊情況，我們可以排序後由小至大挨個遍歷，用 (i + 1, i) 這樣用比較大的元素攻擊小的元素直到最後剩下一個元素。

對於 $\le m$ 的情況，可以把最後 m - 1 的元素與前 m - 1 的元素進行配對，剩下中間的元素就用 m = 1 的情況進行操作。

最後剩下 m = 0 的特殊情況，這種情況要考慮的點會是當有一個元素特別大時，剩下的元素都攻擊後仍然無法使其生命值小於 0 ，這種情況會不合法。

接著構造答案的部分，一開始我的想法是由小到大的排序中第 n - 1 的元素往前遍歷，只要元素和加上第 1 個元素能夠超過第 n 個元素便可以構造出消滅最大元素的方法，最後再把剩下元素用 m = 1 的情況操作一遍，最後調整順序即可，但是這個方法一直卡 testcase 2。

最後參考別人的方法，$1 \le m \le \lfloor \frac{n}{2} \rfloor$ 的部分跟之前一樣，但是 m = 0 的情況由另一邊出發。

由小至大遍歷元素不斷地將最大的元素減去當前元素，直到最大元素小於等於次大元素，那剩下的元素使用 m = 1 情況的操作，最後的攻擊會使最大和次大的生命值都小於 0。