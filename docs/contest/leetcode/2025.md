...
Q4. Maximize Cyclic Partition Score (8 points)
...
??? note "Details"
    ...
    * Solution:
        ...
        對於沒有環形的約束下，整體的題目框架是股票交易模型 [3573. 买卖股票的最佳时机 V](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-v/description/)，其中子陣列中只有最大最小值是有意義的其他不關心，可以寫成:

        \begin{array}{ll}
                max\sum_{k}^{i = 1} = |a_{r_i} - a_{l_i}|
            \end{array}
        
        用max轉換成:

        \begin{array}{ll}
            \sum_{k}^{i = 1} = max(a_{r_i} - a_{l_i}, a_{l_i} - a_{r_i})
        \end{array}
